# ===================================================================================
# FINAL CODE FOR FAMILY BUDGET APP WITH GOOGLE SHEETS DATABASE (v2)
# ===================================================================================
...
def save_data(df_to_save):
    ...
    try:
        # Convert date columns to string in ISO format for consistent saving
        df_for_gsheet = df_to_save.copy()
        df_for_gsheet['Date'] = pd.to_datetime(df_for_gsheet['Date']).dt.strftime('%Y-%m-%d')
        # Handle NaT in Deadline before converting to string
        df_for_gsheet['Deadline'] = pd.to_datetime(df_for_gsheet['Deadline']).dt.strftime('%Y-%m-%d')
        df_for_gsheet.fillna('', inplace=True)  # Replace any remaining NaNs with empty strings

        # Clear the sheet before writing to avoid duplicates
        worksheet.clear()
        # Use gspread-dataframe to set the dataframe, including headers
        set_with_dataframe(worksheet, df_for_gsheet, include_index=False, include_sheet_name=False, resize=True)
...
if df_expenses.empty:
    ...
    # --- THIS IS THE FIX ---
    # Standardize the Deadline column to ensure it's a proper datetime or NaT
